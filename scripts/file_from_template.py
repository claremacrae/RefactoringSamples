...
class FileFromTemplate(object):
    ...
    def create_file(self, refactoring) -> None:
    ...
    def render_file_template(self, refactoring, template_name) -> str:
        template = FileFromTemplate.env.get_template(template_name)
        return self.render_template(refactoring, template)

    def render_string_template(self, refactoring, template_text) -> str:
        template = Template(template_text)
        return self.render_template(refactoring, template)

    def render_template(self, refactoring, template) -> str:
